...
#cycles = ['00','03','06','12','15','18']
#cycles = ['06','09','12','21']
path = '/scratch/nkim/logFilesFrom4dvar_v22'
#out_path = '/scratch/nkim/logFilesFrom4dvar_v22/JOstatistics'
out_path = '/scratch/nkim/'
keep_JOtables = True
figs = True
Variable = 'U'
Obstype = 'AMV'
...
def extractJOtable(fileLog, fileJOtable):
    ...
    with open (fileLog, 'rt') as myfile:
        ...
        # add its contents to mylines.
            # looks for the start of the 1st 4DVminim
             if( "Diagnostic JO-table (JOT) MINIMISATION JOB T0089 NCONF=   131 NSIM4D=     0 NUPTRA=     0" in myline):
                in_JOtable = True
             elif("End of JO-table (JOT)" in myline) :
                in_JOtable = False

            # looks for the end of the 1st 4DVminim
             elif ("Diagnostic JO-table (JOT) MINIMISATION JOB T0089 NCONF=   131 NSIM4D=   999 NUPTRA=     0"  in myline):
                in_JOtable = True
             elif("End of JO-table (JOT)" in myline) :
                in_JOtable = False

            # looks for the 1st trajectory run
             if ("Diagnostic JO-table (JOT) TRAJECTORY JOB   T0539 NCONF=     1 NSIM4D=     0 NUPTRA=     1" in myline):
                in_JOtable = True
             elif("End of JO-table (JOT)" in myline) :
                   in_JOtable = False

            # looks for the end of the 2nd 4DVminim
             elif ("Diagnostic JO-table (JOT) MINIMISATION JOB T0179 NCONF=   131 NSIM4D=   999 NUPTRA=     1"  in myline):
                in_JOtable = True
             elif("End of JO-table (JOT)" in myline) :
                in_JOtable = False

            # looks for the 2nd trajectory run
             if ("Diagnostic JO-table (JOT) TRAJECTORY JOB   T0539 NCONF=     1 NSIM4D=     0 NUPTRA=     2" in myline):
                in_JOtable = True
             elif("End of JO-table (JOT)" in myline) :
                   in_JOtable = False

             #### add all the relevant JOT in a txt file (can be saved or not       
             if in_JOtable:
                 fh.write(myline)
    fh.close()
####
...
def plotJo(label,path, Jo, obs, num, cty, var):
    ...
    ax.plot(Jo["start4DVmn1"].astype(float),label='start4DVmn1',linestyle='solid',color='blue')
    ax.plot(Jo["4DVmn2"].astype(float), label = 'end4DVmn2',linestyle='solid',color='orange')
    ax.plot(Jo["4DVtr2"].astype(float), label= '4DVtr2',linestyle='solid',color='red')
    ax.set_ylim(0,1.5)
    ax.set_ylabel("Jo/n")
    ax2 = ax.twinx()
