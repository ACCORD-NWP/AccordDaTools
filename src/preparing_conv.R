...
#$~> odbsql -T -q 'SELECT rad2deg(lat),rad2deg(lon),an_depar,fg_depar,obsvalue FROM hdr,body,desc where obstype=1 AND an_depar IS NOT NULL'
lab_input.x = c("lat","lon","an_depar","fg_depar","obsvalue")

# READ DATA
file = paste(read,"/ccma_mon_conv_", yymmddnt, sep="")
#------------------------------------
# RUN PREPARATION
#------------------------------------
if (file.exists(file)){ 
  ...
  # SAVE STATISTICS
  save(StatTab, file=paste(save,"/stat_conv_",yymmddnt,".Rdat",sep=""))
