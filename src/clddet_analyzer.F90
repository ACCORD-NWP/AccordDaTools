...
program clddet_analyzer
  ...
  character(len=20) :: instrument_name, width,thres
  integer :: iargc
...
! Find out which instrument settings we are looking for
  if (iargc()<3) then
     ...
     write (*,'(a)') 'and/or I_WINDOW_WIDTH and/or R_BT_THRES'
     stop
  endif
  ...
  read (width,'(I10)') i_window_width
  read (thres,'(F6.2)') r_bt_thres
...
! Sort the data
  ...
  if (.not. l_sorted) stop
     ...
     write (*,'(A,I8)') 'i_num_chans ',i_num_chans
     write (*,'(A,I5)') 'i_window_width ',i_window_width
     write (*,'(A,F6.2)') 'r_bt_threshold', r_bt_thres
